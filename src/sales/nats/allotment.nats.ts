...
// --- Allotment Detail (nested in create request) ---

export interface AllotmentDetailInput {
  roomTypeId: string;
  roomTypeName: string;
  allottedRooms: number;
  daysOfWeek?: number[] | null; // [1,3,5] = Mon,Wed,Fri; null = all days
}
...
export interface CreateAllotmentNatsRequest {
  ...
  hotelId: string;
  partnerId: string;
  partnerType: string; // 'CORPORATE' | 'TRAVEL_AGENT'
  partnerName: string;
  contractNumber?: string;
  startDate: string; // ISO date
  endDate: string;
  releaseDays: number;
  inventoryControl: string; // 'ELASTIC' | 'FIXED'
  ratePlanId?: string;
  notes?: string;
...
export interface AllotmentDailyInventoryResponse {
  ...
  stopSell: boolean;
  available: number; // allottedRooms - pickedUpRooms - releasedRooms
}
...
export interface AllotmentSummaryResponse {
  ...
  totalPickedUpRooms: number;
  utilizationPercent: number;
  upcomingReleases: number; // rooms releasing in next 7 days
}
...
// --- NatsResponse wrappers ---

export type CreateAllotmentNatsResult = NatsResponse<AllotmentResponse>;
export type FindAllotmentsNatsResult = NatsResponse<{ allotments: AllotmentResponse[]; total: number; page: number; limit: number; totalPages: number }>;
export type GetAllotmentNatsResult = NatsResponse<AllotmentResponse>;
export type GetAllotmentPickupNatsResult = NatsResponse<{ daily: AllotmentDailyInventoryResponse[] }>;
