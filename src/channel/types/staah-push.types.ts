...
export interface StaahPushRoom {
  id: string;                   // room type ID (STAAH external room ID)
  ...
  max_children?: string;
  child_age?: string;
  roomstaystatus: string;       // "new" | "modified" | "request" | "cancelled"
  ...
  extracomponents?: Array<{ name?: string; value?: string }>;
  taxes?: Array<{ name?: string; value?: string }>;
  withheldtaxes?: Array<{ name?: string; value?: string }>;
  info?: string;
  facilities?: string;
...
export interface StaahPushReservation {
  id: string;                          // e.g. "352832131_GRANDHN"
  hotel_id: string;                    // STAAH property_id (e.g. "GRANDHN")
  hotel_name?: string;
  reservation_notif_id: string;        // ACK identifier
  channel_booking_id: string;          // OTA booking reference
  status: string;                      // "new" | "modified" | "cancelled" | "request"
  booked_at?: string;
  modified_at?: string;
  processed_at?: string;
  currencycode: string;
